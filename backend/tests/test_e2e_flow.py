...
def test_marketplace_core_flow(mysql_client):
    ...
    seller_me = mysql_client.get("/api/auth/me", headers=auth_header(seller_token))
    ...
    db = SessionLocal()
    try:
        digital_category = db.query(Category).filter(Category.name == "数码").one()
        category_id = digital_category.id
    finally:
        db.close()

    init_asset = mysql_client.post(
    ...
    upload_asset = mysql_client.post(
    ...
    complete_asset = mysql_client.post(
        "/api/media/complete",
        headers=auth_header(seller_token),
    ...
    create_product = mysql_client.post(
        "/api/products",
        headers=auth_header(seller_token),
            json={
    ...
    pending = mysql_client.get("/api/admin/products/pending", headers=auth_header(admin_token))
    assert pending.status_code == 200
    assert any(item["id"] == product_id for item in pending.json()["data"])

    product_list = mysql_client.get(
        "/api/admin/products",
    ...
    product_context_before = mysql_client.get(
    ...
    resubmit_product = mysql_client.post(
    ...
    product_context_after = mysql_client.get(
        f"/api/admin/products/{product_id}",
        headers=auth_header(admin_token),
    )
    ...
    session_again = mysql_client.post(
        "/api/chat/sessions",
        headers=auth_header(buyer_token),
        json={"product_id": product_id, "seller_id": seller_id},
    )
    ...
    complete = mysql_client.post(f"/api/orders/{order_id}/confirm", headers=auth_header(buyer_token))
    assert complete.status_code == 200
    assert complete.json()["data"]["status"] == "COMPLETED"

    review = mysql_client.post(
        "/api/reviews",
        headers=auth_header(buyer_token),
    ...
    orders_after_review = mysql_client.get("/api/orders", headers=auth_header(buyer_token))
    ...
    update_review = mysql_client.post(
        "/api/reviews",
        headers=auth_header(buyer_token),
        json={
            "order_id": order_id,
            "product_review": {"rating": 3, "content": "到手后发现成色一般，已修改评价"},
        },
    )
    ...
    product_reviews = mysql_client.get(f"/api/reviews/products/{product_id}")
    ...
    process_report = mysql_client.post(
    ...
    report_context = mysql_client.get(
    ...
    review_appeal = mysql_client.post(
    ...
    appeal_context = mysql_client.get(
    ...
    recommendations = mysql_client.get(f"/api/recommendations/products/{product_id}/related")
    assert recommendations.status_code == 200
    assert "items" in recommendations.json()["data"]

    first_history = mysql_client.post(
        f"/api/history/products/{product_id}/view",
        headers=auth_header(buyer_token),
    )
    second_history = mysql_client.post(
        f"/api/history/products/{product_id}/view",
        headers=auth_header(buyer_token),
    )
    ...
    history_listing = mysql_client.get("/api/history/recent", headers=auth_header(buyer_token))
